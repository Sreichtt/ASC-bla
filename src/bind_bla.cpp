...
PYBIND11_MODULE(bla, m) {
    ...
    py::class_<Matrix<double, RowMajor>>(m, "Matrix")
        .def(py::init<size_t, size_t>(),
             py::arg("height"), py::arg("width"), "create a matrix of given height and width")
        ...
        if(j<0) j += self.Width();
        if(i<0 || i >= self.Height()) throw py::index_error("Matrix row index out of range");
        if(j<0 || j >= self.Width()) throw py::index_error("Matrix col index out of range");
        self(i,j) = v; })
        .def("__getitem__",
